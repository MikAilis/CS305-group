...
def process_inbound_udp(sock):
    ...
    if Type == 0:  # WHOHAS
        ...
        print(f'chunk length: {len(ex_received_chunk[ex_downloading_chunkhash])}, real length {len(chunkid_peer)}')
            ...
            if Ack > now_session.sendBase:  # send new data
                ...
                while now_session.nextPktNum <= now_session.sendBase + now_session.cwnd - 1:
                    send_data = config.haschunks[chunkhash][
                                (now_session.nextPktNum - 1) * MAX_PAYLOAD: now_session.nextPktNum * MAX_PAYLOAD]
                    ...
                    print(
                        f'chunkid: {chunkid}, seq = {now_session.nextPktNum}. I send data pkt to {from_addr} with payload: {bytes.hex(send_data)}')
                ...
                if now_session.duplicate_acks == 3:
                    # fast retransmit
                    send_data = config.haschunks[chunkhash][
                                (now_session.sendBase - 1) * MAX_PAYLOAD: now_session.sendBase * MAX_PAYLOAD]
                    ...
                    print(
                        f'chunkid: {chunkid}, seq = {now_session.sendBase}. I send data pkt to {from_addr} with '
                        f'payload: {bytes.hex(send_data)}')
            ...
            for chunkid, now_session in s_sessions.items():
                if now_session.isTimeout():  # timeout retransmit
                    ...
                    send_data = config.haschunks[chunkhash][
                                (now_session.sendBase - 1) * MAX_PAYLOAD: now_session.sendBase * MAX_PAYLOAD]
                    retransmit_pkt = struct.pack("!HBBHHIIIQ", 52305, 93, 3, HEADER_LEN, HEADER_LEN + len(send_data),
                                                 now_session.sendBase, 0, chunkid,
                                                 int(round(time() * 1000000))) + send_data
                    print(
                        f'chunkid: {chunkid}, seq: {now_session.sendBase}.I retransmit pkt to {now_session.peer_addr}')
                    now_session.open_timer()
